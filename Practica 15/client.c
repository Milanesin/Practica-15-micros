            ...
            case TC_W4_READY:
                switch(hci_event_packet_get_type(packet)) {
                    case GATT_EVENT_NOTIFICATION: {
                        ...
                        if ( value length == 2) {
                            float temp = little endian read
                            16( value , 0) ;

                            printf ( " read temp %.2f degc \ n " ,
                                temp / 100) ;
                        } else {

                            printf ( " Unexpected length %d \ n " ,
                                value length ) ;
                        }
                        break ;
                }      
...
static void hci event handler ( uint8 t packet type ,
    uint16 t channel , uint8 t * packet , uint16 t size ), 
    {
    UNUSED ( size ) ;
    UNUSED ( channel ) ;         
    bd addr t local addr ;
    if ( packet type != HCI EVENT PACKET ) return ;
    
    uint8 t event type = hci event packet get type (
        packet ) ;
    switch ( event type ) {
        case BTSTACK EVENT STATE :
            if ( btstack event state get state ( packet ) ==
            HCI STATE WORKING ) {
                gap local bd addr ( local addr ) ;
                printf ( " BTstack up and running on %s .\ n "
                    , bd addr to str ( local addr ) ) ;
                    client start () ;
                    } else {
state = TC OFF ;
...
if (!advertisement_report_contains_service(
    ORG_BLUETOOTH_SERVICE_ENVIRONMENTAL_
    SENSING, packet)) return;

// store address and type
gap_event_advertising_report_get_address(
    packet, server_addr);
server_addr_type = gap_event_advertising_
    report_get_address_type(packet);
...
switch (hci_event_le_meta_get_subevent_code(
    packet)) {
    case HCI_SUBEVENT_LE_CONNECTION_COMPLETE:
        if (state != TC_W4_CONNECT) return;
        connection_handle = hci_subevent_le_
            connection_complete_get_
            connection_handle(packet);
        // initialize gatt client context
        // with handle, and add it to the
        // list of active clients
        // query primary services
        DEBUG_LOG("Search for env sensing"
            " service.\n");
        state = TC_W4_SERVICE_RESULT;
        gatt_client_discover_primary_
            services_by_uuid16(handle_gatt_
            client_event, connection_
            handle, ORG_BLUETOOTH_SERVICE_
...
case HCI_EVENT_DISCONNECTION_COMPLETE:
    ...
    if (listener_registered){
        ...
        gatt_client_stop_listening_for_
...
static void heartbeat_handler(struct btstack_timer_
    source *ts) {
    // Invert the led
    ...
    // Restart timer
    btstack_runloop_set_timer(ts, (led_on || quick_
        flash) ? LED_QUICK_FLASH_DELAY_MS : LED_SLOW_
        FLASH_DELAY_MS);
    btstack_runloop_add_timer(ts);
...
// is fine to call btstack_run_loop_execute() but equally you can continue executing user code.
...
#else
    // this core is free to do its own stuff except when using 'polling' method (in which case you should use
