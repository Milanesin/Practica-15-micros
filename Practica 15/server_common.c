...
static uint8_t adv_data[] = {
    ...
    // Name
    0x17, BLUETOOTH_DATA_TYPE_COMPLETE_LOCAL_NAME, 'P', 'i', 'c', 'o', '-', 'T', 'e', 'm', 'p', '-', 'S', 'e', 'n', 's', 'o', 'r', '-', '0', '0',
...
int att_write_callback(hci_con_handle_t connection_handle,
    uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size) {
...
void poll_temp(void) {
    ...
    // Scale raw reading to 16 bit value using a Taylor
    //    expansion (for 8 <- bits <- 16)
    uint16_t raw16 = raw32 << (16 - bits) | raw32 >> (2 * bits - 16);

    // ref https://github.com/raspberrypi/pico-
    //     micropython-examples/blob/master/adc/
    //     temperature.py
    const float conversion_factor = 3.3 / (65535);
    float reading = raw16 * conversion_factor;

    // The temperature sensor measures the Vbe voltage
    //    of a biased bipolar diode, connected to the
    //    fifth ADC channel
    // Typically, Vbe = 0.706V at 27 degrees C, with a
